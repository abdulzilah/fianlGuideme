...
class CallPageSt extends StatefulWidget {
  /// non-modifiable channel name of the page
  final String channelName,email;

  /// Creates a call page with given channel name.
  const CallPageSt({Key key, this.channelName, this.email}) : super(key: key);

  @override
  _CallPageStateSt createState() => _CallPageStateSt(email : email);
}



class _CallPageStateSt extends State<CallPageSt> {
  String email;

  _CallPageStateSt({this.email});
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          RawMaterialButton(
            ...
            onPressed: () => _onCallEnd(context, email),
            child: Icon(
              Icons.call_end,
              color: Colors.white,
              size: 35.0,
  ...
  void _onCallEnd(BuildContext context, String s) {
    Navigator.pop(context);
    get(s);
   // _showRatingDialog();
  }

  void incSession(String email, int sessions, int total) async {
    await databaseReference.collection("Student")
        .document(email)
        .updateData({
      "num_of_weekly_sessions": sessions + 1,
      "num_of_sessions": total + 1
    }
    );
    print(sessions);
  }
  ...
  void get(String email) {
    ...
    Firestore.instance.collection('Student')
      ...
      incSession(email, sessions, total);
    });
  }
